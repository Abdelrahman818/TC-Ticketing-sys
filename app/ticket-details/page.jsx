...
import { assignTicket, changeTicketStatus, deleteTicket, getTicketById, loadAssignableUsers, updateTicket } from '@/lib/tickets';
                ...
                comments.map((comment) => {
                  ...
                  return (
                    <div key={comment._id || comment.id} className="flex gap-3 group">
                      {/* Avatar */}
                      <div className={`flex-shrink-0 h-8 w-8 rounded-full ${avatarColor} flex items-center justify-center text-[11px] font-bold text-white shadow-sm`}>
                        {initials}
                      </div>
                      {/* Bubble */}
            ...
            <div className="border-t border-border bg-slate-50/60 px-5 py-4">
              <div className="flex gap-3">
                <div className="flex-1 relative">
                  <textarea
                    className="w-full min-h-[76px] resize-none rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition shadow-sm"
