...
import { API_ROUTES, apiRequest, getStoredToken } from '@/config';
...
export function subscribeToTicketEvents(onEvent) {
  const controller = new AbortController();
  let reconnectTimer;

  const connect = async () => {
    try {
      const response = await fetch(API_ROUTES.tickets.events, {
        headers: {
      ...
      if (!response.ok || !response.body) {
        throw new Error(`Ticket event stream failed with status ${response.status}`);
      }
...
export async function uploadTicketPhoto(ticketId, photoFile, caption = '') {
  ...
  const payload = await apiRequest(API_ROUTES.tickets.photos(ticketId), {
    method: 'POST',
    body: formData,
  });
...
export async function deleteTicketPhoto(ticketId, photoId) {
  await apiRequest(API_ROUTES.tickets.photo(ticketId, photoId), {
    method: 'DELETE',
  });
}
...
export function useTickets() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      window.setTimeout(handleInitialLoad, 0);
      window.addEventListener('tickets:changed', handleTicketChange);
      const unsubscribe = subscribeToTicketEvents(handleTicketChange);
      return () => {
        window.removeEventListener('tickets:changed', handleTicketChange);
        unsubscribe();
      };
    } else {
